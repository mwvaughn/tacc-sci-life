...
Source:    https://github.com/broadinstitute/picard/archive/%{major_version}.%{minor_version}.%{patch_version}.tar.gz
%package %{PACKAGE}
Summary: %{shortsummary}
Group:   Applications/Life Sciences
%description package
...
#rm -Rf *
rm -rf %{pkg_base_name}
git clone --branch %{pkg_version} https://github.com/broadinstitute/picard.git
cd picard ; ./gradlew shadowJar --stacktrace -p=$PWD
cd ../ ; chmod -R go=u-w picard/build/
cp -r picard/build $RPM_BUILD_ROOT/%{INSTALL_DIR}/
...
java jvm-args -jar $%{MODULE_VAR}_DIR/build/libs/picard.jar
