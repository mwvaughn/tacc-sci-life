...
module load python 
# add the next line when mpi4py is installed for intel 15
# module load mpi4py
...
yes | ./configure intel # --with-netcdf $TACC_NETCDF_DIR intel
make install

make clean
yes | ./configure -mpi intel # --with-netcdf $TACC_NETCDF_DIR intel
make install

make clean
yes | ./configure -cuda intel # --with-netcdf $TACC_NETCDF_DIR intel
make LDFLAGS="-Wl,-rpath,$TACC_CUDA_LIB" install

make clean
yes | ./configure -cuda -mpi intel # --with-netcdf $TACC_NETCDF_DIR intel
make LDFLAGS="-Wl,-rpath,$TACC_CUDA_LIB" install
