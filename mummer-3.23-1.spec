...
%define shortsummary MUMmer - A modular system for the rapid whole genome alignment of finished or draft sequence
Summary: %{shortsummary}

# Give the package a base name
%define pkg_base_name mummer
...
Source:    MUMmer%{pkg_version}.tar.gz
...
make CC=icc CXX=icpc CFLAGS="-O3 %{TACC_OPT}" CXXFLAGS="-O3 %{TACC_OPT}"

# Manually fix the paths for MUMmer. This means the RPM will not be relocatable
# but the underlying scripts are too dumb for this to work otherwise.
OWD=$PWD
PERL_SCRIPTS=$(find . -type f -exec grep -Il "/bin/perl" {} \;)
for X in $PERL_SCRIPTS
do
    echo $X
    sed -i "s|$OWD|\$ENV{'%{MODULE_VAR}_DIR'}|g" $X
done

CSH_SCRIPTS=$(find . -type f -exec grep -Il "/bin/csh" {} \;)
for X in $CSH_SCRIPTS
do
    echo $X
    sed -i "s|$OWD|\$%{MODULE_VAR}_DIR|g" $X
done
