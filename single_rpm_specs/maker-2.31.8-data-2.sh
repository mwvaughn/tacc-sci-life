...
# Augustus can be built from source but now requires bamtools
# Need to update the build instructions. For now, have built it by hand
# cd ${MAKER_DATADIR}
...
echo "Ownership(root) and group(G-800657) must be changed manually as root"

echo "chown -R root:G-800657 $MAKER_DATADIR"
echo "chmod -R 755 $MAKER_DATADIR"
