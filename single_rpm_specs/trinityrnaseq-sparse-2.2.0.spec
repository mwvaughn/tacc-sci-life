%define PNAME trinityrnaseq-sparse
Version: 2.2.0
Release: 1
...
ml samtools/1.3 perl sparsehash
...
---
> CXXFLAGS        = -I${INCLUDE} ${LOCAL_CFLAGS} -O3 -xAVX -axCORE-AVX2
> CXX            = icpc
EOF
## Patch plugins
...
>  INCHWORM_CONFIGURE_FLAGS = CXX=icpc CXXFLAGS="-D__GOOGLE__ $(IFLAGS)" LDFLAGS="-Wl,-rpath,$(IOMP)"
...
help (
[[
%{PNAME} has a lower memory footprint than the original trinityrnaseq
during inchworm, but is 3 times slower on average. We recommend
starting with the original version and then using this if memory issues
are encountered.
...
Usage of %{PNAME} is the exact same as trinityrnaseq.
...
family("trinity")
always_load("perl","bowtie/1.1.2","samtools/1.3")
EOF
## Module File End
#--------------------------------------
